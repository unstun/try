...
# Allow duplicate OpenMP runtimes (MKL + PyTorch) to coexist on Windows.
# Without this, libiomp5md.dll conflicts and the process aborts before training starts.
...
if __name__=="__main__":
    ...
    writer.add_text("device", f"{device} ({device_name})")
    ...
    progress_bar = None
    interrupted = False
    interrupt_ckpt_path = None

    try:
        progress_bar = tqdm(range(args.train_episode), desc="Training episodes", dynamic_ncols=True)
        for i in progress_bar:
            ...
            if len(reward_list) > 0:
                recent_success_rate = np.mean(succ_record[-100:]) if len(succ_record) > 0 else 0
                recent_reward = np.mean(reward_list[-10:])
                progress_bar.set_postfix({
                    "episode": i + 1,
                    "success_rate_100": f"{recent_success_rate:.2f}",
                    "avg_reward_10": f"{recent_reward:.2f}",
                })
    except KeyboardInterrupt:
        interrupted = True
        interrupt_ckpt_path = f"{save_path}SAC_interrupt_{time.strftime('%Y%m%d_%H%M%S')}.pt"
        parking_agent.save(interrupt_ckpt_path, params_only=True)
        print(f"\nKeyboardInterrupt detected. Checkpoint saved to {interrupt_ckpt_path}")
    finally:
    ...
    if not interrupted:
        ...
        final_ckpt_path = f"{save_path}SAC_final.pt"
        parking_agent.save(final_ckpt_path, params_only=True)
        print(f"Training complete. Final checkpoint saved to {final_ckpt_path}")

    if interrupt_ckpt_path is None and interrupted:
        interrupt_ckpt_path = f"{save_path}SAC_interrupt.pt"
    if interrupted:
        print(f"Training interrupted. Latest checkpoint: {interrupt_ckpt_path}")
