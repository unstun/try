...
class MultiObsEmbedding(nn.Module):
    ...
    def load_img_encoder(self, path, device, require_grad = False):
        """
        Autoencoder checkpoints were saved with torch.save(obj) so we need full
        unpickling here. PyTorch 2.6 switched torch.load default
        weights_only=True, which blocks loading custom classes like AE_Conv.
        Explicitly set weights_only=False (and fall back for older versions)
        to keep the previous behavior.
        """
        try:
            ae = torch.load(path, map_location=device, weights_only=False)
        except TypeError:  # older torch without weights_only argument
